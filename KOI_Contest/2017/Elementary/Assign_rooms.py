...
단, 세 종류의 방은 모두 충분한 개수가 있다고 가정하며, 위의 예에서와 같이 세 종류의 방을 모두 활용하지 않고
한 종류 또는 두 종류의 방만 이용하여 배정하는 것도 허용한다.
'''
...
# Decision whether assign rooms are possible or impossible
def is_possible(A, B, C, N):
    # One trial
    if N % C == 0 or N % B == 0 or N % A == 0:
        return 1
    else:
        # Two trial
